    ...
    namespace player {
        ...
        static constexpr auto shoot_1 = [](TWorld &world, components::Emission &emitter, const components::Movement movement, const double time, const bullet::UpdateFunction &bullet_function) {
          ...
          static constexpr auto emit = [](auto &world, const bullet::UpdateFunction &bullet_function, const auto movement_component, const auto position, const auto direction, const auto rotation)
          {
              ...
              auto &bulletMovement = get<components::Movement>(emitted_bullet);
              bulletMovement.speed = speed;
              bulletMovement.position = movement_component.position;
              sprite_component.rotation = raylib::Vector2(0.0f, 0.0f).Angle(rotation);
              bulletMovement.position = bulletMovement.position.Add(position);
              bulletMovement.direction = direction;

              world.player_bullets.add(emitted_bullet);
          };
        ...
        static constexpr auto kPlayerNormalBehaviour = [](const double time, TWorld &world, components::Sprite &sprite, components::Movement &movement, components::Emission &emission) {
